		...
		for (int ix = 0; ix < TERRAIN_NX; ix++)
		{
			...
			if (((ix + iz) & 1) == 0)
			{
				...
				if (d < t)
				{
					t = d;
					vec3 hitPos = rayOrigin + rayDirection * t;
					vec3 alb = terrainAlbedo(hitPos.xz, hitPos.y, triRand);
					vec3 nDetail = grassMicroNormal(hitPos.xz, triN);
					if (dot(nDetail, rayDirection) > 0.0) nDetail *= -1.0;
					float sunVis = SunsetSunVisibility(hitPos, nDetail);
					vec3 col = shadeSunset(alb, nDetail, rayDirection, sunVis);
					hitNormal = nDetail;
					hitColor = alb;
					hitEmission = col;
					hitType = LIGHT;
				...
				if (d < t)
				{
					t = d;
					vec3 hitPos = rayOrigin + rayDirection * t;
					vec3 alb = terrainAlbedo(hitPos.xz, hitPos.y, triRand * 0.97);
					vec3 nDetail = grassMicroNormal(hitPos.xz, triN);
					if (dot(nDetail, rayDirection) > 0.0) nDetail *= -1.0;
					float sunVis = SunsetSunVisibility(hitPos, nDetail);
					vec3 col = shadeSunset(alb, nDetail, rayDirection, sunVis);
...
//---------------------------------------------------------------------------------------
{
	...
	for (int iz = 0; iz < TERRAIN_NZ; iz++)
	{
		for (int ix = 0; ix < TERRAIN_NX; ix++)
		{
			...
			{
				d = TriangleIntersect(v00, v10, v11, rayOrigin, rayDirection, triN);
				if (d < t) { t = d; vec3 hitPos = rayOrigin + rayDirection * t; vec3 alb = terrainAlbedo(hitPos.xz, hitPos.y, triRand); vec3 nDetail = grassMicroNormal(hitPos.xz, triN); if (dot(nDetail, rayDirection) > 0.0) nDetail *= -1.0; hitNormal = nDetail; hitColor = alb; hitEmission = vec3(0.0); hitType = DIFF; hitObjectID = float(objectCount); }
				objectCount++;

				d = TriangleIntersect(v00, v11, v01, rayOrigin, rayDirection, triN);
				if (d < t) { t = d; vec3 hitPos = rayOrigin + rayDirection * t; vec3 alb = terrainAlbedo(hitPos.xz, hitPos.y, triRand * 0.97); vec3 nDetail = grassMicroNormal(hitPos.xz, triN); if (dot(nDetail, rayDirection) > 0.0) nDetail *= -1.0; hitNormal = nDetail; hitColor = alb; hitEmission = vec3(0.0); hitType = DIFF; hitObjectID = float(objectCount); }
				objectCount++;
			}
			else
			{
